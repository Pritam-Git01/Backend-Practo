...
app.delete("/fdata", async function (req, res) {
  let data = await featureData.deleteOne({
    _id: { $eq: "64b84c0017b128b24e1e7157" },
  });
  console.log(data);
  return res.send(data);
});
...
app.delete("/consult", async function (req, res) {
  let data = await consultationData.deleteOne({
    _id: { $eq: "64bc263af3d93c20edf6ef19" },
  });
  console.log(data);
  return res.send(data);
});
app.put("/specialist", async function (req, res) {
  let data = await specialistData.updateOne(
    { _id: { $eq: "64bc17c877d506288a19787d" } },
    { $push: { tags: "Vaginal infection" } }
  );
  console.log(data);
  return res.send(data);
});
...
app.get("/consult/:id", async function (req, res) {
  let data = await consultationData.find({
    tags: { $regex: `^${req.params.id}`, $options: "i" },
  });
  return res.send(data);
});

app.get("/consulting/:id", async function (req, res) {
  let data = await consultationData.findOne({
    doctor: { $regex: `^${req.params.id}`, $options: "i" },
  });
  return res.send(data);
});
...
app.put("/consult", async function (req, res) {
  let data = await consultationData.updateOne(
    { _id: { $eq: "64c0f9f2c930bf2e043ad441" } },
    { $push: { tags: "Cold cough & fever" } }
  );
  console.log(data);
  return res.send(data);
});
